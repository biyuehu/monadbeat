(function () {
    ...
    function getVisibleQuiz() {
        var active = (window.FilterPanel && window.FilterPanel.getActiveTypes)
            ? window.FilterPanel.getActiveTypes()
            : {};
        var out = [];
        for (var i = 0; i < allQuiz.length; i++) {
            var p = allQuiz[i];
            if (Object.prototype.hasOwnProperty.call(active, p.problem_type) && !active[p.problem_type]) continue;
            out.push(p);
        }
        return out;
    }
    ...
    function renderList(list) {
        ...
        statusText.textContent = (shown === total)
            ? ('Total ' + total + ' questions')
            : ('Showing ' + shown + ' / ' + total + ' questions');
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            html += '<div class="problemItem" data-index="' + index + '">' +
                '<span class="p-status">' + icon + '</span>' +
                '<span class="p-id">' + p.id + '</span>' +
                '<span class="p-title">' + title + '</span>' +
                '<div class="p-tags"><span class="p-tag">' + typeLabel + '</span></div>' +
                '<span class="p-diff">Difficulty: ' + stars(diff) + '</span>' +
                '<span class="p-state ' + STATE_CLASS[state] + '">' + STATE_MAP[state] + '</span>' +
                '</div>';
        }
        grid.innerHTML = html;
    ...
    function loadProblems() {
        ...
        Promise.all([loadDoneIds(), fetch(url).then(function (r) {
            if (!r.ok) throw new Error('HTTP ' + r.status);
            return r.json();
        })])
    ...
    window.addEventListener('filter:change', function () {
