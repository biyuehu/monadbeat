(function() {
    ...
    function renderInfo() {
        ...
        return '' +
            ...
            '</div>' +
            '<div class="infoGrid">' +
                '<div class="infoKey">Username</div><div class="infoVal mono">' + esc(u.username) + '</div>' +
                '<div class="infoKey">Email</div><div class="infoVal">' + esc(u.email) + '</div>' +
                '<div class="infoKey">User ID</div><div class="infoVal mono">' + esc(u.uid) + '</div>' +
                '<div class="infoKey">Joined</div><div class="infoVal">' + esc(u.joined) + '</div>' +
                '<div class="infoKey">Level</div><div class="infoVal mono">' + esc(u.level) + '</div>' +
                '<div class="infoKey">Solved</div><div class="infoVal">' + DONE_IDS.length + ' / ' + u.total + '</div>' +
                '<div class="infoKey">Score</div><div class="infoVal mono">' + u.score + '</div>' +
                '<div class="infoKey">Rank</div><div class="infoVal">#' + u.rank + '</div>' +
                '<div class="infoKey">Bio</div><div class="infoVal">' + esc(u.bio) + '</div>' +
                '<div class="infoKey">Last Active</div><div class="infoVal">' + esc(u.lastActive) + '</div>' +
            '</div>';
    }
        ...
        if (LEADERBOARD_DATA.length === 0) {
            ...
            for (var i = 0; i < LEADERBOARD_DATA.length; i++) {
                ...
                html += '<div class="' + cls + '">' +
                    ...
                    '<span class="rankUser">' + esc(r.name) + (isMe ? '  ← me' : '') + '</span>' +
                    '<span class="rankSolved">✔ ' + r.solved + ' solved</span>' +
                    '<span class="rankScore">' + r.score + ' pt</span>' +
                    '</div>';
            }
        }
    ...
    function renderDone() {
        ...
        var html = '' +
            '<div class="viewHeader">' +
                ...
                '<span class="viewSub">' + DONE_DATA.length + ' / ' + total + ' Solved</span>' +
            '</div>' +
            '<div class="doneList">';
        ...
        if (DONE_DATA.length === 0) {
            ...
            for (var i = 0; i < DONE_DATA.length; i++) {
                ...
                html += '<div class="doneRow">' +
                    ...
                    '<span class="doneId">#' + d.id + '</span>' +
                    '<span class="doneTitle">' + esc(d.title) + '</span>' +
                    '<span class="doneType">' + esc(TYPE_LABELS[d.type] || d.type) + '</span>' +
                    '<span class="doneStars">' + stars(d.diff) + '</span>' +
                    '</div>';
            }
        }
        ...
        return '' +
            ...
            '<div class="themeSection">' +
                ...
                '<div class="themeOptions">' +
                    '<button type="button" class="themeCard' + (cur === 'light' ? ' active' : '') + '" data-theme="light">' +
                    ...
                    '</button>' +
                    '<button type="button" class="themeCard' + (cur === 'dark' ? ' active' : '') + '" data-theme="dark">' +
    ...
    function pickCurrentUsername() {
        ...
        for (var k in ALL_USERS.users) {
            if (Object.prototype.hasOwnProperty.call(ALL_USERS.users, k)) {
    ...
    function renderAvatar(user) {
        ...
        img.onerror = function () {
            ...
            span.textContent = user && user.username
    ...
    function loadAllData() {
        var pUsers = loadJSON('../datas/users.json').catch(function () { return {}; });
        var pBoard = loadJSON('../datas/leaderboard.json').catch(function () { return { leaderboard: [] }; });
        var pQuiz  = loadJSON('../datas/quiz.json').catch(function () { return { quiz: [] }; });

        return Promise.all([pUsers, pBoard, pQuiz]).then(function (results) {
            ALL_USERS = results[0] || {};
