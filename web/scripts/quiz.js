;(() => {
  ...
  function highlight(code) {
    ...
    while (i < n) {
      ...
      if (/[A-Za-z_]/.test(ch)) {
        ...
        } else if (Object.hasOwn(TYPES, word)) {
  ...
  function render(index) {
    ...
    progressInfo.innerHTML =
      '<span class="done">' + (index + 1) + '</span> <span class="total">/ ' + quiz.length + '</span>'
  ...
  function renderChoice(q) {
    ...
    for (var i = 0; i < opts.length; i++) {
      ...
      html +=
        '<label class="choiceOption" data-index="' +
        i +
        '">' +
  ...
  function showComplete() {
    ...
    feedbackContainer.innerHTML =
      ...
      '<div class="completeSub">You solved all ' +
      quiz.length +
      ' questions!</div>' +
  ...
  function handleSubmit() {
    ...
    if (q.problem_type === 'predict_behavior') {
      if (selected === -1) {
        showFeedback('⚠️ Please choose an option!', 'hint')
        return
      }
      submitting = true
    ...
    if (ans.trim() === '') {
      showFeedback('⚠️ Please enter your answer!', 'hint')
      typeInput.focus()
      return
    }
